...
async function submitEdit(){if(!editForm.barcode||!editForm.expiryDate)return ElMessage.warning('Barcode 和有效期不能为空');saving.value=true;try{await updateExpiryRecord(id,{...editForm});ElMessage.success('修改成功');editVisible.value=false;await load()}catch(e:any){ElMessage.error(e.message||'修改失败')}finally{saving.value=false}}
...
async function process(status:'NORMAL'|'PROMOTE'|'DAMAGE'){try{await ElMessageBox.confirm(`确定处理为“${processLabel(status)}”吗？`,'处理确认',{type:'warning'});await processExpiryRecord(id,status);ElMessage.success('处理成功');await load()}catch(e:any){if(e!=='cancel'&&e!=='close')ElMessage.error(e.message||'处理失败')}}
...
<div v-loading="loading">
  <div class="page-title"><div><h2>有效期详情</h2><p>记录 #{{ id }}</p></div><div><el-button @click="router.back()">返回</el-button><el-button v-if="record?.confirmStatus==='UNCONFIRM'" type="warning" @click="openConfirm">确认</el-button><template v-if="record && record.confirmStatus!=='UNCONFIRM' && record.processStatus==='UNPROCESS'"><el-button type="success" @click="process('NORMAL')">正常销售</el-button><el-button type="warning" @click="process('PROMOTE')">打折</el-button><el-button type="danger" @click="process('DAMAGE')">报损</el-button></template><el-button type="primary" @click="openEdit">修改</el-button><el-button type="danger" @click="remove">删除</el-button></div></div>
